...
[CustomEditor(typeof(SSRFeature))]
public class SSREditor : Editor
{
    ...
    {
        public static GUIContent TracingMode = EditorGUIUtility.TrTextContent("Trace Mode",
            "Linear Tracing: Uses less memory at the cost of performance and visual quality. HiZ Tracing: Higher visual quality and performance at the cost of memory.");

        public static GUIContent Downsample = EditorGUIUtility.TrTextContent("Downsample",
            "1 / (value + 1) = resolution scale. Higher values increase performance at the cost of visual quality.");

        public static GUIContent StepStrideLength =
            EditorGUIUtility.TrTextContent("Step Length", "Raymarch step length (IMPACTS VISUAL QUALITY).");

        public static GUIContent MaxSteps =
            EditorGUIUtility.TrTextContent("Max Steps", "Maximum length of a raycast (IMPACTS PERFORMANCE).");

        public static GUIContent ReflectSky =
            EditorGUIUtility.TrTextContent("Reflect Sky", "Considers the sky as part of the reflection.");

        public static GUIContent MinSmoothness = EditorGUIUtility.TrTextContent("Min Smoothness",
            "Minimum smoothness value needed for SSR to be applied.");

        public static GUIContent DitherType = EditorGUIUtility.TrTextContent("Dither Type",
            "Dithering Type. Interleaved Gradient for TAA. 8x8 for everything else.");
