...
public static class GlobalSSRSettings
{
    const string GlobalScaleShaderString = "_SSRGlobalScale";
...
public class SSRFeature : ScriptableRendererFeature
{
    ...
    internal class SsrPass : ScriptableRenderPass
    {
        ...
        public override void Configure(CommandBuffer cmd, RenderTextureDescriptor cameraTextureDescriptor)
        {
            ...
            GlobalSSRSettings.GlobalResolutionScale = 1.0f / Scale;
            if (IsPadded)
            {
                ScreenHeight = cameraTextureDescriptor.height * GlobalSSRSettings.GlobalResolutionScale;
                ScreenWidth = cameraTextureDescriptor.width * GlobalSSRSettings.GlobalResolutionScale;
            ...
            cmd.GetTemporaryRT(reflectionMapID, Mathf.CeilToInt(PaddedScreenWidth), Mathf.CeilToInt(PaddedScreenHeight),
                0, FilterMode.Point, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Default, 1, false);

            tempPaddedSourceID = Shader.PropertyToID("_TempPaddedSource");
            int tx = (int)(IsPadded
                ? Mathf.NextPowerOfTwo((int)cameraTextureDescriptor.width)
                : cameraTextureDescriptor.width);
            int ty = (int)(IsPadded
                ? Mathf.NextPowerOfTwo((int)cameraTextureDescriptor.height)
    ...
    internal class SSRSettings
    {
        ...
        [Range(0, 1)] public uint downSample = 0;
    ...
    internal static SSRFeature ssrFeatureInstance;
...
#if UNITY_EDITOR
        var d = UniversalRenderPipelineDebugDisplaySettings.Instance.AreAnySettingsActive;
        if (!d)
        {
